...
export default function ListModulesScreen() {
  ...
  const renderModule = ({ item }: { item: Module }) => (
    <TouchableOpacity
      onPress={() => router.push(`/cabinets/detail?id=${item.id}`)}
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        ...
        <View className="flex-row items-center justify-between mb-8">
          <View className="flex-1 mr-4" style={{minWidth: 0}}>
            <Text className="text-4xl font-bold text-foreground mb-2" numberOfLines={1}>Armarios</Text>
        ...
        {loading ? (
          ...
          <View className="flex-1 items-center justify-center gap-4">
            ...
            <Text className="text-sm text-muted text-center">
              Crea tu primer módulo para verlo aquí
            </Text>
            <TouchableOpacity
